...
HINSTANCE VisualManager::m_s_hinstRes = nullptr;
...
BOOL __stdcall VisualManager::SetStyle(eStyle Style_in, LPCTSTR lpszPath)
{
	...
	if (::FindResource(hinstRes, strStyle, AFX_RT_STYLE_XML) == nullptr)
	{
#if !defined _AFXDLL
...
void VisualManager::OnUpdateSystemColors()
{
	...
	HINSTANCE hinstResOld = nullptr;

	if (m_s_hinstRes == nullptr)
	{
...
BOOL VisualManager::OnNcActivate(CWnd* pWnd, BOOL bActive)
{
	...
	if (IsOwnerDrawCaption())
	{
		...
		{
			::RedrawWindow(((CMDIFrameWnd *)pWnd)->m_hWndMDIClient, nullptr, nullptr, RDW_INVALIDATE | RDW_ALLCHILDREN);
		}
	}
...
void VisualManager::DrawNcBtn(CDC* pDC, const CRect& rect, UINT nButton, AFX_BUTTON_STATE state, BOOL bSmall, BOOL bActive, BOOL bMDI/* = FALSE*/)
{
	...
	CMFCToolBarImages* pImage = nullptr;
	...
	const CString& strDocument, HICON hIcon, BOOL bPrefix, BOOL bActive, BOOL bTextCenter, const CObList& lstSysButtons)
{
	...
	if (hIcon != nullptr)
	{
		...
		::DrawIconEx(memDC.GetSafeHdc(), x, y, hIcon, szIcon.cx, szIcon.cy, 0, nullptr, DI_NORMAL);
	...
	for (POSITION pos = lstSysButtons.GetHeadPosition(); pos != nullptr;)
	{
...
BOOL VisualManager::NcPaint(CWnd* pWnd, const CObList& lstSysButtons, CRect rectRedraw)
{
	...
	if (dc.GetSafeHdc() != nullptr)
	{
		...
		if (bActive && IsOwnerDrawCaption() && pWnd->IsKindOf(RUNTIME_CLASS(CMDIChildWnd)))
		{
			...
			if (pParent)
			{
				CMDIChildWnd *pActiveChild = pParent->MDIGetActive(nullptr);
				if (pActiveChild)
				{
		...
		if (pStatusBar->GetSafeHwnd() != nullptr && pStatusBar->IsWindowVisible())
		{
			CMFCStatusBar* pClassicStatusBar = DYNAMIC_DOWNCAST(CMFCStatusBar, pStatusBar);
			if (pClassicStatusBar != nullptr)
			{
...
BOOL VisualManager::OnSetWindowRegion(CWnd* pWnd, CSize sizeWindow)
{
	...
	if (DYNAMIC_DOWNCAST(CMFCPopupMenu, pWnd) != nullptr)
	{
		sz  = CSize(3, 3);
	}
	else if (DYNAMIC_DOWNCAST(CMFCRibbonBar, pWnd) != nullptr)
	{
		return FALSE;
	}
	else
	{
		...
		{
			pWnd->SetWindowRgn(nullptr, TRUE);
			return TRUE;
		}
...
{
	if (!CanDrawImage() ||/* (pBar != nullptr && pBar->IsDialogControl()) || */m_ToolBarGripper.GetCount() == 0)
	{
...
void VisualManager::OnDrawSeparator(CDC* pDC, CBasePane* pBar, CRect rect, BOOL bHorz)
{
	...
	if (!CanDrawImage() || pBar == nullptr/* || pBar->IsDialogControl()*/)
	{
	...
	if (!bHorz)
	{
		...
		if (bPopupMenu && !bIsRibbon && !pBar->IsKindOf(RUNTIME_CLASS(CMFCColorBar)))
		{
			...
			if (bPopupMenu)
			{
				...
				if (pWnd != nullptr && pWnd->IsKindOf(RUNTIME_CLASS(CMFCPopupMenu)))
				{
...
void VisualManager::OnFillButtonInterior(CDC* pDC, CMFCToolBarButton* pButton, CRect rect, CMFCVisualManager::AFX_BUTTON_STATE state)
{
	...
	if (pCustButton == nullptr)
	{
		...
		if (pMenuButton != nullptr && pBar != nullptr)
		{
		...
		else if (pBar != nullptr && pBar->IsKindOf(RUNTIME_CLASS(CMFCColorBar)))
		{
			if (bChecked)
			{
				pRenderer = &m_ctrlMenuItemBack;
		...
		else if (pBar != nullptr && pBar->IsKindOf(RUNTIME_CLASS(CMFCOutlookBarToolBar)))
		{
			bHandled = TRUE;
		}
...
void VisualManager::OnDrawButtonBorder(CDC* pDC, CMFCToolBarButton* pButton, CRect rect, CMFCVisualManager::AFX_BUTTON_STATE state)
{
	...
	CMFCToolBarMenuButton* pMenuButton = DYNAMIC_DOWNCAST(CMFCToolBarMenuButton, pButton);
	if (pMenuButton == nullptr || !pMenuButton->IsDroppedDown())
	{
		return;
	}

	BOOL bIsPopupMenu = pMenuButton->GetParentWnd() != nullptr && pMenuButton->GetParentWnd()->IsKindOf(RUNTIME_CLASS(CMFCPopupMenuBar));
	...
	if (pPopupMenu != nullptr && (pPopupMenu->IsWindowVisible() || pPopupMenu->IsShown()) && !pPopupMenu->IsRightAlign() && !(pPopupMenu->GetExStyle() & WS_EX_LAYOUTRTL))
	{
		ExtendMenuButton(pMenuButton, rect);

		CDrawingManager dm(*pDC);

		dm.DrawShadow(rect, m_nMenuShadowDepth, 100, 75, nullptr, nullptr, m_clrMenuShadowBase);
	}
}
...
COLORREF VisualManager::OnFillMiniFrameCaption(CDC* pDC, CRect rectCaption, CPaneFrameWnd* pFrameWnd, BOOL bActive)
{
	...
	if (DYNAMIC_DOWNCAST(CMFCBaseToolBar, pFrameWnd->GetPane()) != nullptr)
	{
		bActive = FALSE;
		bIsToolBar = TRUE;
	}
...
COLORREF VisualManager::GetToolbarButtonTextColor(CMFCToolBarButton* pButton, CMFCVisualManager::AFX_BUTTON_STATE state)
{
	...
	if (pButton->GetParentWnd() != nullptr && pButton->GetParentWnd()->IsKindOf(RUNTIME_CLASS(CMFCMenuBar)))
	{
	...
	else
	{
...
#endif // _AFXDLL
		{
			if (hIcon != nullptr)
			{
				...
				pDC->DrawState(rectImage.TopLeft(), rectImage.Size(), hIcon, DSS_NORMAL, (HBRUSH) nullptr);
			}
			else
			{
				const CImageList* pImageList = pTabWnd->GetImageList();
				if (pImageList != nullptr && uiIcon != (UINT)-1)
				{
					//----------------------
...
void VisualManager::OnEraseTabsButton(CDC* pDC, CRect rect, CMFCButton* pButton, CMFCBaseTabCtrl* pBaseTab)
{
	...
	if (!CanDrawImage() || pWndTab == nullptr/* || pBaseTab->IsDialogControl()*/)
	{
...
void VisualManager::OnDrawTasksGroupCaption(CDC* pDC, CMFCTasksPaneTaskGroup* pGroup, BOOL bIsHighlighted /*= FALSE*/, BOOL bIsSelected /*= FALSE*/, BOOL bCanCollapse /*= FALSE*/)
{
	...
	// ---------------------------
	BOOL bShowIcon = (pGroup->m_hIcon != nullptr && pGroup->m_sizeIcon.cx < rectGroup.Width() - rectGroup.Height());
	if (bShowIcon)
	{
...
{
	if (!CanDrawImage() || m_brMainClientArea.GetSafeHandle() == nullptr)
	{
...
COLORREF VisualManager::OnDrawMenuLabel(CDC* pDC, CRect rect)
{
	...
	pDC->FillRect(rect, m_brGroupBackground.GetSafeHandle() != nullptr ? &m_brGroupBackground : &m_brBarBkgnd);
...
BOOL VisualManager::DrawPushButton(CDC* pDC, CRect rectClient, CMFCButton* pButton, UINT uiState)
{
	...
	bool IsDefault = (pParent != nullptr && pParent->IsDefaultButton(pButton->GetDlgCtrlID()));
...
{
	WCHAR wszLoadPath[MAX_PATH + 1] = { '\0' };
...
bool VisualManager::DisableDwmNcRendering(HWND hWnd_in)
{
	...
	if (hDLL != nullptr)
	{
		...
		if (m_pfDwmSetWindowAttribute != nullptr)
		{
