...
CSetDPI * VisualDialog::m_pDPI = nullptr;
...
VisualDialog::VisualDialog(UINT TemplateID_in, CWnd* pParentWnd_in)
	: CDialogEx(TemplateID_in, pParentWnd_in), m_pImageList(nullptr),
	  m_IconResID(0), m_hTransparentBrush(nullptr), m_IconBackground(-1),
	  m_hIcon(nullptr), m_IconBorder(-1), m_DefaultButton(IDOK), // 1 => IDOK
...
VisualDialog::~VisualDialog()
{
	...
	if (m_hTransparentBrush != nullptr)
	{
		DeleteObject(m_hTransparentBrush);
		m_hTransparentBrush = nullptr;
	}
...
BOOL VisualDialog::OnInitDialog(void)
{
	...
	if (m_hIcon == nullptr && m_IconResID > 0)
	...
	SetWindowPos(nullptr, -1, -1, m_WindowRect.Width() - m_FrameWidth, m_WindowRect.Height() - m_FrameHeight,
...
void VisualDialog::SetDefaultButton(UINT ButtonResID_in)
{
	...
	if (MsgResult != 0L)
	{
		...
		if (ResID != ButtonResID_in)
		{
			...
			if (pNewDefault != nullptr && pCurrentDefault != nullptr)
			{
...
{
	if (pButton_in != nullptr && ::IsWindow(pButton_in->GetSafeHwnd()))
	{
...
{
	if (pButton_in != nullptr)
		return (DYNAMIC_DOWNCAST(CMFCButton, pButton_in) != nullptr);
...
{
	if (m_pImpl != nullptr && m_pImpl->OnActivate(nState_in, pWndOther_in, bMinimized_in) == FALSE)
...
{
	if ((m_pDPI == nullptr || ForceAttach_in) && VisualManager::IsDPI(DPI_in) == false)
	{
		if (m_pDPI == nullptr)
			m_pDPI = new CSetDPI;
