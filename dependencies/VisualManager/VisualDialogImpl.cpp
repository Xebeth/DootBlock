...
VisualDialogImpl::VisualDialogImpl(VisualDialog& Dialog_in)
	: CDialogImpl(Dialog_in), m_Dialog(Dialog_in), m_pFocusedCtrl(nullptr),
...
BOOL VisualDialogImpl::OnActivate(UINT nState_in, CWnd* pWndOther_in, BOOL bMinimized_in)
{
	switch(nState_in)
	{
		...
		case WA_CLICKACTIVE:
		case WA_ACTIVE:
			if (m_pFocusedCtrl != nullptr && ::IsWindow(m_pFocusedCtrl->GetSafeHwnd()))
...
/*! \brief Retrieves a pointer to the caption button
	...
	\return a pointer to the caption buttons under the mouse 
			or nullptr if the mouse isn't over any of the caption buttons
	\sa VisualDialogImpl::OnNcHitTest
*/
CMFCCaptionButtonEx* VisualDialogImpl::GetSysButton(UINT nHitTest_in)
{
	for (POSITION pos = m_lstCaptionSysButtons.GetHeadPosition(); pos != nullptr;)
	{
...
void VisualDialogImpl::OnTrackCaptionButtons(CPoint MousePos_in)
{
	...
	if (pBtn != nullptr && pBtn->m_bEnabled)
	{
...
void VisualDialogImpl::InitializeCaptionButtons(void)
{
	...
	if (m_lstCaptionSysButtons.IsEmpty())
	{
		...
		HMENU hSysMenu = nullptr;

		// check if the system menu is valid
		if (pSysMenu != nullptr && ::IsMenu(pSysMenu->m_hMenu))
		{
...
{	
	...
	if (m_Dialog.m_hIcon != nullptr)
	{
		...
		::DrawIconEx(WindowDC.GetSafeHdc(), x, y, m_Dialog.m_hIcon, szIcon.cx, szIcon.cy, 0, nullptr, DI_NORMAL);
...
{
	ASSERT(pCalcSizeParams != nullptr);
	BOOL Result;
