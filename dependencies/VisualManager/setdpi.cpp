...
#pragma pack(pop)
	...
	static DLGITEMTEMPLATE* FindFirstDlgItem(const DLGTEMPLATE* pTemplate)
	{
		...
		// Everything else means that this is a nullptr terminated Unicode string
		// which identifies the menu resource
		if (*pw == 0xFFFF)
			pw += 2;				// Has menu ID, so skip 2 words
		else
			while (*pw++);			// Either No menu, or string, skip past terminating nullptr
		...
		if (*pw == 0xFFFF)
			...
			while (*pw++);			// Either No class, or string, skip past terminating nullptr
...
void CSetDPI::Attach(HINSTANCE hInst,HWND _hwnd,int _IDD,double dpi, bool relative)
{
	int t;
	...
	double x_n=szf.cx, x_d=4, y_n=szf.cy, y_d=8;
	...
	if (relative)
	{
		rect.right=(int)(lpDialogTemplate->cx*x_n/x_d+0.5);
		rect.bottom=(int)(lpDialogTemplate->cy*y_n/y_d+0.5);
	}
	else
	{
