...
VisualTabCtrl::VisualTabCtrl(VisualTabDialog *pParent_in)
	: m_CurrentTabIndex(0), m_pCurrentTab(nullptr), m_HeaderSize(0, 0),
...
bool VisualTabCtrl::InsertTab(UINT TabIndex_in, UINT TabHeaderID_in, UINT TabPageID_in, const TCHAR *pHeaderText_in)
{
	...
	if (pHeaderText_in == nullptr || CreateHeaderWindow(pTabHeader, TabHeaderID_in, pHeaderText_in))
	{
...
/*! \brief Retrieves the tab object at the specified index
	...
	\return a pointer to the tab if found; nullptr otherwise
*/
...
/*! \brief Retrieves the tab object at the specified index
	\param[in] TabIndex_in : the index of the tab to retrieve
	\return a const pointer to the tab if found; nullptr otherwise
*/
...
bool VisualTabCtrl::SetActiveTab(UINT TabIndex_in)
{
	...
	if (pTab != nullptr)
	{
		...
		if (pHeader != nullptr && IsWindow(pHeader->GetSafeHwnd()))
		{
			// don't switch tabs if we're already on the one we want
			if (m_CurrentTabIndex != TabIndex_in || m_pCurrentTab == nullptr)
			{
				TabPage *pCurrentPage = nullptr;

				// uncheck the current header
				if (m_pCurrentTab != nullptr)
				...
				if (m_pCurrentTab != nullptr && (pCurrentPage = m_pCurrentTab->GetPage()) != nullptr)
				{
					pCurrentPage->OnDeactivate();
					pCurrentPage->ShowWindow(SW_HIDE);
				}
				// retrieve and display the new one
				if ((pCurrentPage = GetPage(TabIndex_in)) != nullptr)
				{
...
void VisualTabCtrl::ResizeHeaders(UINT Height_in)
{
	...
	for (Iter = m_Tabs.begin(); Iter != m_Tabs.end(); ++Iter)
	{
		...
		// move the header
		pHeader->SetWindowPos(nullptr, HeaderRect.left, HeaderRect.top,
