...
TabPage::TabPage(UINT Width_in, UINT Height_in, UINT TabIndex_in)
	: m_TabIndex(TabIndex_in), m_MandatoryID(0), m_HeaderHeight(0),
	  m_ContainerRect(0, 0, Width_in, Height_in), m_hBkgColor(nullptr),
	  m_hTransparentBrush(nullptr), m_pDPI(nullptr)
{
					 ...
					 UINT HeaderHeight_in, UINT nIDTemplate_in)
{
	...
	if (CDialogBar::Create(pParentWnd_in, nIDTemplate_in, WS_CHILD, nIDTemplate_in))
	{
		...
		SetWindowPos(nullptr, ContainerRect_in.left + 1, ContainerRect_in.top + 1,
...
{
	if (m_pDPI == nullptr && VisualManager::IsDPI(DPI_in) == false)
	{
		m_pDPI = new CSetDPI;
